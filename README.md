...
## Genome Alignment and Variant Calling 
The assembly was aligned against the reference genome Salmonella enterica serovar Typhimurium str. LT2 reference genome with accession number (GCF_000006945.2) gotten from NCBI using Minimap2. Visual inspection of the alignment in IGV (Integrative Genomics Viewer) demonstrated strong structural agreement across the chromosome. Despite the high structural similarity, variant calling performed with SVIM identified specific genetic divergences. A visual inspection in IGV (Integrative Genomics Viewer) confirmed the presence of Single Nucleotide Polymorphisms (SNPs) and small insertions/deletions (Indels) distinguishing the sequenced strain from the reference (Figure 3). In this figure the most prominent feature is a ~40 kb insertion (40,084 bp) present in the assembled genome but absent in the reference. A second large insertion of approximately 33 kb (33,693 bp) was observed downstream. The presence of these large insertions is corroborated by the SVIM-asm variant track (blue bars), which flagged these regions as high-confidence structural variants.

![image_alt](https://github.com/Sodjoh/Genomics/blob/47c16010dea898c9d7ee67bc2221d065d0c6cd02/Screenshot%202026-02-06%20122927.png)
